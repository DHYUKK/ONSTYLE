...
<script src="/resources/consmr/codi/myDressRoom/js/virtualCodi.js?32232"></script>	
<script src="/resources/consmr/codi/myDressRoom/js/virtualCodiCRUD.js?1"></script>
<style>
.item_info{
...
<body>
	...
	<form id="codiInsertForm" name="codiInsertForm">
		...
		<input type="hidden" name="codi_seq" value="${myDrsRmVO.codi_seq}">
		<input type="hidden" name="readPageNme" value="${myDrsRmVO.readPageNme}">
		...
		<div id="collage_container" style="height: 80%;">
			...
			<div id="canvas" style="float:left; width:42%; height:91%;">
			 	...
			 	<!-- 멘트 입력란 -->
			 	<c:if test="${myDrsRmVO.readPageNme eq 'reqst'}">
					<div class="col-lg-12" id="mentBox" style = "margin-top:75%; width:100%; z-index:5">
						<textarea class="form-control" rows="2" id="ment" name="codi_reqst_contnt" placeholder="요청 사항을 입력해주세요!" 
						style="border:1px solid #e5e5e5; background-color:white; text-align: center; font-size: 25px;" >${reqstVO.codi_reqst_contnt}</textarea>
					</div>
				</c:if>
			</div>
